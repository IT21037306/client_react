...
const Profile = () => {
  ...
  //   const getUser = () => {
  //     axios
  //       .get("http://localhost:8090/user/" + loggedUser.email)
  //       .then((res) => {
  //         setUser(res.data);
    ...
    const getUser = async () => {
      fetch("http://localhost:8090/auth/login/success", {
        ...
        .then((resObject) => {
          ...
          setFName(resObject.user.family_name);
          setEmail(resObject.user.email);
          setOldEmail(resObject.user.email);
          setLName(resObject.user.given_name);
  ...
  const uploadImage = async (event) => {
    ...
    console.log(base64);
    axios
      .post("http://localhost:8090/uploadImage", { image: base64 })
      .then((res) => {
        setUrl(res.data);
        axios.patch(
          `http://localhost:8090/user/update/${loggedUser.email}`,
          { url: res.data },
          config
        );
  ...
  const handleSubmit = async (e) => {
    ...
    await axios
      .patch(
        `http://localhost:8090/user/update/${oldEmail}`,
        updateDetails,
        config
      )
      .then(() => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8090/membership/email/${loggedUser.email}`, config)
      .then((res) => {
        setId(res.data._id);
  ...
  const validateInputs = () => {
    ...
    if (!phoneNumber) {
      ...
      errors.phoneNumber =
        "Phone number must contain only numbers and be at least 10";
    }
      ...
      <Grid container spacing={2} padding={2}>
        ...
        <Grid item xs={12} sm={6} md={4} lg={3} sx={{ display: display }}>
          ...
          >
            ...
            <a href={qr} download="Membership QR Code">
              <img src={qr} />
            </a>
            <br />
        ...
        <Grid item xs={12} sm={6} md={6} lg={6}>
          <ContentBox
            backgroundColor={theme.palette.background.alt}
            sx={{ textAlign: "center", display: display }}
          >
            <img
              style={{ width: "100px", height: "100px", marginTop: "50px" }}
              src="/assets/memberlogo.png"
            ></img>
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            <TextField
              sx={{ width: "calc(100% - 80px)", margin: "10px 40px" }}
              label="Email"
              value={email}
            ...
            <Button
              ...
              color="success"
              variant="contained"
              sx={{ margin: "10px 40px", width: "calc(100% - 80px)" }}
            >
              Confirm Update
            </Button>
          </form>
